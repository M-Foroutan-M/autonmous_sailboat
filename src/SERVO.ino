...
// RC Backup Setup - These are the Rudder and Sail connections to the Arduino
#define RC_RUDDER_PIN 2  // Rudder channel (CH1)
#define RC_SAIL_PIN 3    // Sail channel (CH3)


// Servo pulse width limits - This is one of the important parts of the RC Controller system.
// The pulse width signals between receiver and transmitter typically range from 900 to 2200 microseconds.
// If the pulse extends beyond 2200 ( or falls below 900), the Arduino will retain the last valid.
// This process ensures stable and reliable control of the boat, preventing unexpected movements and maintaining safe operation.
...
// PWM driver channels - The receiver sends commands to the rudder and sail,
// which are connected to channel 1 (rudder) and channel 2 (sail) respectively.
...
// Autonomous Setup - This is the most important part of the code because here the target location coordinates (latitude and longitude)
// are defined. Once the GPS module starts receiving signals from the satellites, the three key libraries begin working together:
// TinyGPSPlus gps; - for decoding and handling GPS data / Adafruit_PWMServoDriver pwm = Adafruit_PWMServoDriver();
// - for controlling the servos (rudder and sail) / File dataFile; - for logging data to the SD card
...
// RC ISRs - Interrupt Service Routines written to handle RC signals coming from the receiver,
// used to read pulse widths accurately and in real time.
void rudder_isr() {
...
// A setup function used to initialize and configure all hardware modules (GPS, compass, wind sensor, RC pins, PWM driver, and SD card) before starting the main loop.
//It ensures each component is ready to operate correctly when the boat begins running.
void setup() {
 Serial.begin(115200);
 Wire.begin();
...
// The main loop function continuously runs part of the code that decides whether to operate in manual RC mode or autonomous mode, based on valid RC pulse signals.
// It checks the latest RC inputs and switches control logic accordingly, ensuring real-time responsiveness.
void loop() {
...
// Manual Mode - This part is very important because if the boat loses GPS signals or is unable to navigate autonomously,
//The RC controller can be turned on to take control. When activated, it sends pulses to the receiver,
// which the Arduino reads and uses to adjust the rudder and sail positions in real time, allowing manual override of the system.
...
// Autonomous mode enables fully automatic navigation from point A to point B by continuously reading GPS and compass data, adjusting rudder and sail in real time.
// Once the target is reached, the boat stops and holds position.
...
// Support Functions - This section includes helper functions to read GPS, compass, and wind data, calculate target bearing and distance, and control rudder and sail angles.
// it also logs data to SD card and prints debug information.
void readGPS() {
...
void logToSD() {
 ...
 if (dataFile) {
   dataFile.println("================================");
   dataFile.println("Date: 10/07/2025");  // <-- Manually typed date
